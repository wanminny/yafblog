...
class BaseModels{
	...
	public function __construct() {
		...
		//从子类中定义提取
		$this->table_name   = $this->db_config[$this->db_setting]['prefix'].$this->table_name;
		$this->table_prefix = $this->db_config[$this->db_setting]['prefix'];
	...
	final public function listinfo($where = '', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 10,$urlrule = '',$array = array(), $data = '*') {
		...
		$array = array();
		if ($this->number > 0) {
            $array['page'] = $this->pages;
            $array['data'] = $this->select($where, $data, "$offset, $pagesize", $order, '', $key);
			return  $array;
		} else {
			return array();
		}
	...
	final public function query($sql) {
		$sql = str_replace('yafcms_', $this->db_tablepre, $sql);
	 ...
	 * 						为字符串时[例：`name`='yafcms',`hits`=`hits`+1]。
	 *						为数组时[例: array('name'=>'yafcms','password'=>'123456')]
