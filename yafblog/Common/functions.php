 ...
 */
if ( ! function_exists('auto_load_func'))
{
	function auto_load_func($path = '') {
		
		if (empty($path)) $path = DIRECTORY_SEPARATOR.'Common'.DIRECTORY_SEPARATOR.'autoload';
		
		$path .= DIRECTORY_SEPARATOR.'*.php';
		
		$auto_funcs = glob(APPLICATION_PATH.$path);
...
if ( ! function_exists('load_config'))
{
	...
	function load_config($file, $key = '', $default = '', $reload = false) {
	   ...
	   $path =  APPLICATION_PATH.DIRECTORY_SEPARATOR.'conf'.DIRECTORY_SEPARATOR.$file.'.php';
...
function sys_auth($string, $operation = 'ENCODE', $key = '', $expiry = 0) {
	$key_length = 4;
	$key = md5($key != '' ? $key : load_config('system', 'auth_key'));
	$fixedkey = md5($key);
 ...
 * show_msg('登录成功', array('默认跳转地址'=>'http://www.yafblog.com'));
...
function show_msg($msg, $url_forward = 'goback', $ms = 1250) {
	...
	include(APPLICATION_PATH.'/'.APP_NAME.'/Error.php');
...
function error_h($_error){
         echo '
         
     
         
         <span style="font-size:12px;border:1px solid #F0F0F0;"><b style="color:red">致命错误:</b><b>'.$_error['message'].'</b></br></span>
	     <span style="font-size:12px;border:1px solid #F0F0F0;"><b style="color:red">错误文件:</b>'.$_error['file'].'</br></span>
		 <span style="font-size:12px;border:1px solid #F0F0F0;">错误在第<b>'.$_error['line'].'</b>行</br></span>
	     ...
	     for($i=0;$i<$_error['line'];$i++){
	     	     $code.=$str[$i];
		 }
		 
		 echo '<span style="font-size:12px;text-align:center; margin-left:auto; margin-right:auto;">'.highlight_string($code,true).'</span>';
}
 ...
 * @param $code int   状态码
 * @param $info string 数据
 */
function return_json($msg, $code, $data=''){
 ...
 * 
 */
function seo_friendly_url($params) {
